...
class ClienteList extends TPage
{
    ...
    public function __construct()
    {
        ...
        $column_apelido_id = new TDataGridColumn('apelido->descricao','Apelido','center');
        $column_tipo_pessoa_id = new TDataGridColumn('tipo_pessoa->descricao', 'F/J', 'center');
        ...
        $this->datagrid->addColumn($column_apelido_id);
        ...
        $action_chg_stt = new TDataGridAction(array('MotivoBloqueioClienteForm','onEdit'));
        ...
        $action_chg_stt->setParameter('tipo','C');
        ...
        $action_group->addHeader('Situação');
        $action_group->addAction($action_chg_stt);
        $action_group->addAction($action_hst);
    ...
    public function onHistorico($param)
    {
        ...
        $criteria->add(new TFilter('tipo_bloqueio_id','=',1));
    ...
    public function onSearch()
    {
        ...
        TSession::setValue('ClienteList_filter_razao_social',   NULL);
        TSession::setValue('ClienteList_filter_nome_fantasia',   NULL);
        TSession::setValue('ClienteList_filter_cpf_cnpj',   NULL);
        TSession::setValue('ClienteList_filter_situacao',   NULL);
        ...
        if (isset($data->razao_social) AND ($data->razao_social)) {
            ...
            TSession::setValue('ClienteList_filter_razao_social',   $filter); // stores the filter in the session
        }
        ...
        if (isset($data->nome_fantasia) AND ($data->nome_fantasia)) {
            ...
            TSession::setValue('ClienteList_filter_nome_fantasia',   $filter); // stores the filter in the session
        }


        if (isset($data->cpf_cnpj) AND ($data->cpf_cnpj)) {
            $filter = new TFilter('cpf_cnpj', '=', "$data->cpf_cnpj"); // create the filter
            TSession::setValue('ClienteList_filter_cpf_cnpj',   $filter); // stores the filter in the session
        }


        if (isset($data->situacao) AND ($data->situacao)) {
            $filter = new TFilter('situacao', '=', "$data->situacao"); // create the filter
            TSession::setValue('ClienteList_filter_situacao',   $filter); // stores the filter in the session
        }
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            if (TSession::getValue('ClienteList_filter_razao_social')) {
                $criteria->add(TSession::getValue('ClienteList_filter_razao_social')); // add the session filter
            }


            if (TSession::getValue('ClienteList_filter_nome_fantasia')) {
                $criteria->add(TSession::getValue('ClienteList_filter_nome_fantasia')); // add the session filter
            }


            if (TSession::getValue('ClienteList_filter_cpf_cnpj')) {
                $criteria->add(TSession::getValue('ClienteList_filter_cpf_cnpj')); // add the session filter
            }


            if (TSession::getValue('ClienteList_filter_situacao')) {
                $criteria->add(TSession::getValue('ClienteList_filter_situacao')); // add the session filter
            }
    ...
    /**
     * Efetua a mudanca de status
     */
    public function onMudancaStatus ($param) 
    {
        try {
            TTransaction::open('eventos');
            ...
            $this->form_quick = new TForm;
            $this->form_quick->style = 'padding: 20px;';
            
            $nome = new TEntry('nome');
            $nome->setEditable(FALSE);
            $nome->setValue($cliente->razao_social);
            
            $situacao = new TCombo('situacao');
            $situacao->addItems(array('A'=>'Ativo', 'I'=>'Inativo','B'=>'Bloqueado'));
            $situacao->setValue($cliente->situacao);
            
            $motivo = new TEntry('motivo');
            
            
            
            new TInputDialog('Mudar Status',$this->form_quick);
            TTransaction::close();
            $this->onReload();
    ...
    public function onSaveStatus($param)
    {
        try {
        } catch (Exception $e){
            new TMessage('error',$e->getMessage());
