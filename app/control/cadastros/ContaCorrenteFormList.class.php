<?php
/**
 * ContaCorrenteFormList Form List
...
class ContaCorrenteFormList extends TPage
{
    ...
    public function __construct( $param )
    {
        ...
        $descricao_interna = new TEntry('descricao_interna');


        // add the fields
        $this->form->addQuickField('Código', $id,  100 );
        $this->form->addQuickField('Banco', $banco,  200 , new TRequiredValidator);
        $this->form->addQuickField('Agência', $agencia,  200 , new TRequiredValidator);
        $this->form->addQuickField('Conta Corrente', $conta,  200 , new TRequiredValidator);
        $this->form->addQuickField('Descrição', $descricao_interna,  200 , new TRequiredValidator);
        ...
        $column_conta = new TDataGridColumn('conta', 'Conta Corrente', 'left');
        $column_descricao_interna = new TDataGridColumn('descricao_interna', 'Descrição', 'left');
        $column_login = new TDataGridColumn('login','Login','center');
        $column_data_atualizacao = new TDataGridColumn('data_atualizacao','Data','center');
        ...
        $container->add(TPanelGroup::pack('Conta Corrente', $this->form));
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            if (TSession::getValue('ContaCorrente_filter'))
            {
                // add the filter stored in the session to the criteria
                $criteria->add(TSession::getValue('ContaCorrente_filter'));
            }
